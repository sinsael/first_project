...
public class SpawnManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        enemyhealth = new int[enemiesInScene.Length]; // 배열 크기 지정!
        ...
        for (int i = 0; i < enemiesInScene.Length; i++)
        {
            ...
            // 적체력 초기화
            enemyhealth[i] = enemiesInScene[i].GetComponent<Enemy>().health;
        }
    ...
    public void SpawnEnemies()
    {
        for (int i = 0; i < enemiesInScene.Length; i++)
        {
            ...
            // 적체력 초기화
            enemiesInScene[i].GetComponent<Enemy>().health = enemyhealth[i]; // 적체력 초기화
        }
    }
    ...
    public void SpawnLongNotes()
    {
        for (int i = 0; i < longNotesInScene.Length; i++)
        {
            ...
            longNotesInScene[i].GetComponent<BoxCollider2D>().enabled = true; // 롱노트 콜라이더 활성화
        }
    }
